...
Cambiar `Enable guest self-service sign up via user flows` a `Yes` y seleccionar **Save**.

No cierres inmediatamente la pantalla. Espera a que la operación confirme guardado y vuelve a leer el valor. Debe permanecer `Yes` al refrescar/reingresar.
...
Ir a `Entra ID → External Identities → User flows`. Debe existir `New user flow`.
...
### “No veo User flows”
Revisar `tenant correcto → workforce tenant → self-service habilitado → permisos`.
...
Captura o registro sanitizado que demuestre nombre del tenant, self-service habilitado, menú User flows disponible y comprensión de rol/permisos.
...
→ Continúa con [Etapa 10 · Preparar Identity Providers](./10-self-service-identity-providers.md).
